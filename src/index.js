...
async function api(request, env, path) {
  ...
  if (resource === "ideas") {
    if (method === "GET" && !id) {
      ...
      const rows = (await env.DB.prepare(
        `SELECT i.*, a.name AS area_name, a.emoji AS area_emoji, p.name AS submitter_person_name, z.name AS zone_name,
         ...
         FROM ideas i LEFT JOIN areas a ON a.id = i.area_id LEFT JOIN people p ON p.id = i.submitter_person_id LEFT JOIN zones z ON z.id = i.zone_id
    ...
    if (method === "POST" && !id) {
      ...
      const r = await env.DB.prepare(
        `INSERT INTO ideas (title, description, link, submitter_name, submitter_person_id, area_id, category, thumb, admin_only, tags, zone_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)`
      ).bind(finalTitle, b.description || null, link, b.submitter_name || null, b.submitter_person_id || null, b.area_id || null, b.category || null, b.thumb || null, adminOnly, normalizeTags(b.tags), b.zone_id || null).run();
    ...
    if (method === "PATCH" && id) {
      ...
      await updateRow(env, "ideas", id, pick(b, ["title", "description", "link", "area_id", "category", "stage", "impact", "effort", "decision_note", "admin_only", "tags", "zone_id"]));
      return json({ ok: true });
    }
    if (method === "DELETE" && id) {
  ...
  if (resource === "state" && method === "GET") {
    ...
    const [party, areas, people, tasks, supplies, fb, ni, gu, cl, zn, ev] = await Promise.all([
      ...
      env.DB.prepare("SELECT * FROM events ORDER BY (event_date IS NULL), event_date, sort_order, id").all(),
    ]);
    return json({
  ...
  if (resource === "events") {
    ...
    if (method === "POST") {
      const b = await body(request);
      if (!b.title || !b.title.trim()) return err("Event title required.");
      const r = await env.DB.prepare(
        "INSERT INTO events (title, event_date, start_time, location, notes, sort_order) VALUES (?,?,?,?,?,?)"
      ).bind(b.title.trim(), b.event_date || null, b.start_time || null, b.location || null, b.notes || null, b.sort_order || 0).run();
      return json({ id: r.meta.last_row_id }, 201);
    }
    if (method === "PATCH" && id) {
      const b = await body(request);
      await updateRow(env, "events", id, pick(b, ["title", "event_date", "start_time", "location", "notes", "sort_order"]));
      return json({ ok: true });
    }
    if (method === "DELETE" && id) {
